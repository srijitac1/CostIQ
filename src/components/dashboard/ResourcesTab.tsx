...
export default function ResourcesTab() {
  const resources = ALL_ANOMALIES.filter(a => a.tab === "resources");
  return (
    <div className="space-y-4 animate-fade-up">
      <h3 className="text-sm font-semibold text-foreground">Cloud resource optimization</h3>
      {resources.map(a => (
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wider ${
                  a.risk === "high" ? "bg-cost-amber/15 text-cost-amber" : "bg-cost-blue/15 text-cost-blue"
                }`}>{a.risk}</span>
