...
export default function FinOpsTab() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setFindings(data.filter(a => a.tab === "finops"));
      setLoading(false);
    }
    load();
  }, []);
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <div className="flex items-center justify-between px-2">
        <h3 className="text-[13px] font-bold text-foreground">Financial & Billing Guardrails</h3>
        <span className="text-[11px] text-muted-foreground font-mono opacity-60 uppercase tracking-widest">{findings.length} Invoices Audited</span>
      </div>
      
      {findings.length === 0 && (
        <div className="glass rounded-2xl p-12 text-center border-white/5">
          <p className="text-muted-foreground text-sm">No billing findings. Continuous invoice reconciliation active.</p>
        </div>
      )}
