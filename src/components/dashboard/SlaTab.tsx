...
export default function SlaTab() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setFindings(data.filter(a => a.tab === "sla"));
      setLoading(false);
    }
    load();
  }, []);
  ...
  return (
    <div className="space-y-6 animate-fade-up">
      <div className="flex items-center justify-between px-2">
        <h3 className="text-[13px] font-bold text-foreground">SLA Breach Vulnerabilities</h3>
        <span className="text-[11px] text-muted-foreground font-mono opacity-60 uppercase tracking-widest">{findings.length} Active Contracts Monitored</span>
      </div>
      
      {findings.length === 0 && (
        <div className="glass rounded-2xl p-12 text-center border-white/5">
          <p className="text-muted-foreground text-sm">No SLA risks identified. Response velocity within compliance bounds.</p>
        </div>
      )}
          ...
          <div className="p-6">
            <div className="flex items-start justify-between gap-6">
              ...
              <div className="text-right shrink-0">
                <p className={`text-[20px] font-black font-mono tracking-tighter ${a.risk === "critical" ? "text-primary" : "text-cost-amber"}`}>{fmt(a.impact_annual)}</p>
                <div className="flex flex-col items-end gap-1 mt-1">
                  {a.status === "auto_remediated" || a.status === "approved" ? (
                    ...
                    <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest opacity-60">Penalty Exposure</p>
                  )}
                </div>
              </div>
            </div>
